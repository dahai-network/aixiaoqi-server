    ...
    public class PhoneServerByMySqlServices
    {
        ...
        public static int SetSip_Buddies(string userTel)
        {
            try
            {
                ...
                var result = MySqlDBHelper.ExecuteNonQuery(string.Format(@"Replace INTO sip_buddies (NAME, defaultuser, secret, context,canreinvite, HOST, nat, qualify, TYPE)  VALUES ('{0}', '{0}', '{1}', 'unitoysapp','no', 'dynamic', 'yes', 'yes', 'peer')", userTel, secret));
