...
namespace Unitoys.Services
{
    public class OrderByZCSelectionNumberService : BaseService<UT_OrderByZCSelectionNumber>, IOrderByZCSelectionNumberService
    {
        ...
        public async Task<UT_OrderByZCSelectionNumber> AddOrder(Guid userId, Guid orderByZCId, string Name, string IdentityNumber, string MobileNumber, PaymentMethodType? PaymentMethod)
        {
            using (UnitoysEntities db = new UnitoysEntities())
            {
                ...
                if (zcSelectionNumber != null)
                {
                    ...
                    {
                        order.ZCSelectionNumberId = zcSelectionNumber.ID;
                        order.OrderStatus = OrderByZCSelectionNumberStatusType.SubmitAgo;//设为待提交

                        zcSelectionNumber.OrderByZCSelectionNumberId = order.ID;
        ...
        //        UT_OrderByZCSelectionNumber payOrder = await db.UT_OrderByZCSelectionNumber.Include(x => x.UT_Package).Where(a => a.ID == orderId).SingleOrDefaultAsync();
        ...
        //    {
        //        //根据orderId获取Order对象。
        //        UT_OrderByZCSelectionNumber order = await db.UT_OrderByZCSelectionNumber.Include(x => x.UT_Package).Where(a => a.OrderNum == orderNum).SingleOrDefaultAsync();
