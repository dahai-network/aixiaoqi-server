...
namespace Unitoys.Core
{
    ...
    public class WebHelper
    {
        ...
        public static void SetCookie(string name, string value)
        {
            ...
            if (UTConfig.SiteConfig.SiteDomain != "")
            {
                cookie.Domain = UTConfig.SiteConfig.SiteDomain;
                cookie.Path = "/";
            }
        ...
        public static string GetIP(HttpRequestMessage request)
        {
            ...
            {
                HttpRequestBase httpRequestBase = ((HttpContextWrapper)request.Properties["MS_HttpContext"]).Request;
                if (httpRequestBase.ServerVariables["HTTP_X_FORWARDED_FOR"] != null)
                {
                    ip = httpRequestBase.ServerVariables["HTTP_X_FORWARDED_FOR"];
                }else{
        ...
        public static string GetLocation()
        {
            using(HttpClient client = new HttpClient())
            {
                try
                {
                    var response = client.GetAsync("http://int.dpool.sina.com.cn/iplookup/iplookup.php?format=utf-8")
                    .Result
                    .Content
                    .ReadAsStringAsync()
                    .Result;
                    return response.Replace("\t", "").Replace("-", "").Replace("1", "");
                }
                catch (Exception)
                {
                    return "";
